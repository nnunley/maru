...
struct Symbol	{ char  *bits; int flags; };
...
struct Subr	{ char  *name;  imp_t imp;  proto_t *sig;  int profile; };
...
static oop _newString(size_t char_len)
{
    /* For UTF-8, estimate byte length (worst case: 4 bytes per character) */
    size_t byte_len = char_len * 4 + 1;  /* +1 for null terminator */
    char *gstr= (char *)_newBits(-1, byte_len);			GC_PROTECT(gstr);
    oop	      obj= newOops(String);						GC_PROTECT(obj);
    set(obj, String,size, newLong(char_len));				GC_UNPROTECT(obj);
    set(obj, String,bits, gstr);						GC_UNPROTECT(gstr);
    return obj;
}

static oop newStringN(char *cstr, size_t char_len)
{
    /* For UTF-8, calculate actual byte length of the string */
    size_t byte_len = strlen(cstr);  /* cstr is now UTF-8 */
    oop obj= _newString(char_len);
    memcpy(get(obj, String,bits), cstr, byte_len + 1);  /* +1 for null terminator */
    return obj;
}

static oop newString(char *cstr)
{
    return newStringN(cstr, strlen(cstr));
}
...
{
    return strlen(get(symbol, Symbol,bits));
}
...
static oop newSubr(char *name, imp_t imp, proto_t *sig)
{
...
static oop readExpr(FILE *fp)
{
	...
	switch (c) {
		...
		if (('.' == c) || ('e' == c)) {
		    ...
		    ungetwc(c, fp);
		    oop obj=  newDouble(strtod(buffer_contents(&buf), 0));
		    return obj;
		}
...
static void doprint(FILE *stream, oop obj, int storing)
{
	...
	case Pair: {
		...
		if (head == getVar(globals)) {
		    ...
		    fprintf(stream, "<%s>", get(getHead(getHead(head)), Symbol,bits));
		}
		else
...
//	   }
	default: {
	    oop name= lookup(getVar(globals), intern("%type-names"));
	    if (is(Array, name)) {
		name= arrayAt(name, getType(obj));
		if (is(Symbol, name)) {
		    fprintf(stream, "[34m%s[m", get(name, Symbol,bits));
		    break;
		}
	    }
...
{
    if (nil != src) {
	return fprintf(stream, "%s:%ld", get(car(src), String,bits), getLong(cdr(src)));
    }
    return 0;
}
...
static subr(format)
{
    ...
    char *p, *np;
    oop ans= nil;
    if (!(p= malloc(size))) return nil;  /* UTF-8: size is already in bytes */
    for (;;) {
	...
	if (!(np= realloc(p, size))) {  /* UTF-8: size is already in bytes */
	    free(p);
	    break;
	}
	p= np;
...
static subr(set_string_at)
{
	...
	if (len < 2) len= 2;
	while (len <= idx) len *= 2;
	/* For UTF-8, estimate byte size needed (worst case: 4 bytes per char + 1) */
	set(arr, String,bits, (char *)GC_realloc(get(arr, String,bits), (len + 1) * 4));
...
//   return newLong(strncmp(get(str, String,bits) + off, get(arg, String,bits), len));
// }
...
//   return newLong(strcmp(get(str, Symbol,bits), get(arg, Symbol,bits)));
// }
...
//   char buf[32];
//   swnprintf(buf, 32, "%ld", getLong(arg));
...
//     return newLong(strtol(get(arg, String,bits), 0, 0));
// }
...
//     char buf[32];
//     swnprintf(buf, 32, "%f", getDouble(arg));
//     return newString(buf);
// }
...
static subr(string_double)
{
    ...
    return newDouble(strtod(get(arg, String,bits), 0));
}
...
//	    char *str= (char *)alloca(4 * len + 4);
//	    int	     i;
//	    for (i= 0;	i < len;  ++i) str[i]= get32(in, &tmp32);
//	    str[i]= 0;
//	    //wprintf("loading Symbol %s\n", str);
//	    set(obj, Symbol,bits, strdup(str));
//	    break;
//	}
//	case Subr: {
...
//	    for (i= 0;	i < len;  ++i) str[i]= get32(in, &tmp32);
//	    str[i]= 0;
//	    //wprintf("loading Subr %s\n", str);
//	    set(obj, Subr,name, strdup(str));
//	    set(obj, Subr,imp,	0);
...
//   init_times();
    ...
    s_locals		= intern("*locals*");			GC_add_root(&s_locals		);
    s_set		= intern("set");			GC_add_root(&s_set		);
    s_define		= intern("define");			GC_add_root(&s_define		);
    s_let		= intern("let");			GC_add_root(&s_let		);
    s_lambda		= intern("lambda");			GC_add_root(&s_lambda		);
    s_quote		= intern("quote");			GC_add_root(&s_quote		);
    s_quasiquote	= intern("quasiquote");		GC_add_root(&s_quasiquote	);
    s_unquote		= intern("unquote");			GC_add_root(&s_unquote		);
    s_unquote_splicing	= intern("unquote-splicing");		GC_add_root(&s_unquote_splicing	);
    s_t			= intern("t");				GC_add_root(&s_t		);
    s_dot		= intern(".");				GC_add_root(&s_dot		);
    s_etc		= intern("...");			GC_add_root(&s_etc		);
    s_bracket		= intern("bracket");			GC_add_root(&s_bracket		);
    s_brace		= intern("brace");			GC_add_root(&s_brace		);
    s_main		= intern("*main*");			GC_add_root(&s_main		);

    oop tmp= nil;						GC_PROTECT(tmp);

    globalNamespace= cons(intern("*global-namespace*"), nil);
    globalNamespace= setTail(globalNamespace, cons(globalNamespace, nil));

    globals=		define(globalNamespace, intern("*globals*"), globalNamespace);
    expanders=		define(globalNamespace, intern("*expanders*"),   nil);
    encoders=		define(globalNamespace, intern("*encoders*"),    nil);
    evaluators=		define(globalNamespace, intern("*evaluators*"),  nil);
    applicators=	define(globalNamespace, intern("*applicators*"), nil);

    traceStack=		newArray(32);				GC_add_root(&traceStack);

    backtrace=		define(globalNamespace, intern("*backtrace*"),    nil);
    input=		define(globalNamespace, intern("*input*"),	     nil);
    output=		define(globalNamespace, intern("*output*"),	     nil);
	...
	subr_ent_t *ptr;
	for (ptr= subr_tab;  ptr->name;	 ++ptr) {
	    char *name= strdup(mbs2wcs(ptr->name + 1));
...
// #endif
	...
	arguments= define(globalNamespace, intern("*arguments*"), tmp);
...
//   }
	...
	else if (!strcmp (arg, "-g"))	{ ++opt_g;  opt_p= 0; }
	else if (!strcmp (arg, "-O"))	{ ++opt_O; }
// #  if !defined(WIN32) && (!LIB_GC)
//     else if (!strncmp(arg, "-p", 2)) {
//	opt_g= 0;
//	opt_p= strtoul(arg + 2, 0, 0);
//	if (!opt_p) opt_p= 1000;
